function [d1x,kdx,d1y,kdy]=diff_me_steady_change_speed(rho,theta,number,order,geometry,if_no_x)
%*****************************************************
...
%----------------- set matrix
[d1x_up,kdx_up]=getddiag_steady(rho,number,order,1,0);%上方rho方向的导数系数
[d1x0,kdx0]=getddiag_steady(rho(nr_down:nr),number,order,1,0);%下方rho方向的导数系数
d1x_down=d1x0;
kdx_down=kdx0+nr_down-1;                %极向坐标都是从nt1+1开始，只包含nt_inner个点
%将矩阵转化为正确的格式方便之后在循环中赋值
[d1y0,kdy0]=getddiag_steady(theta(nt1+1:nt1+nt_inner),number,order,2,1);
d1y_inner=d1y0;
kdy_inner=kdy0+nt1;                      %极向坐标都是从nt1+1开始，只包含nt_inner个点
[d1y0,kdy0]=getddiag_steady([theta(t_min:nt1),theta(nt1+nt_inner+1:t_max)-2*pi],number,order,2,0);
d1y_pri=d1y0;
kdy_pri=kdy0+t_min-1+(kdy0>nt1-t_min+1)*nt_inner;    %极向坐标都是从t_min开始，只包含nt_inner个点
[d1y0,kdy0]=getddiag_steady(theta(t_min:t_max),number,order,2,0);
d1y_SOL=d1y0;
...
for i=nr_inner+1:nr
    kk=(t_min:t_max);
    d1y(i,kk,:)=d1y_SOL;   kdy(i,kk,:)=kdy_SOL;
end
for j=nt1+1:nt1+nt_inner%本部分表示的是x点上方区域
    d1x(:,j,:) = d1x_up;        kdx(:,j,:) = kdx_up;
end
for j=[t_min:nt1,nt1+nt_inner+1:t_max]%本部分表示的是x点下方区域
    kk=nr_down:nr;
    d1x(kk,j,:)=d1x_down;  kdx(kk,j,:)=kdx_down;
end
%%
xn_range=10;
order_x=order+2;
%在X点附近使用高阶格式而非稳定格式,附近121个点
[d1x_up,kdx_up]=getddiag_steady(rho,number,order_x,1,0);%上方rho方向的导数系数
[d1x0,kdx0]=getddiag_steady(rho(nr_down:nr),number,order_x,1,0);%下方rho方向的导数系数
d1x_down=d1x0;
kdx_down=kdx0+nr_down-1;                %极向坐标都是从nt1+1开始，只包含nt_inner个点
%将矩阵转化为正确的格式方便之后在循环中赋值
[d1y0,kdy0]=getddiag_steady(theta(nt1+1:nt1+nt_inner),number,order_x,2,1);
d1y_inner=d1y0;
kdy_inner=kdy0+nt1;                      %极向坐标都是从nt1+1开始，只包含nt_inner个点
[d1y0,kdy0]=getddiag_steady([theta(t_min:nt1),theta(nt1+nt_inner+1:t_max)-2*pi],number,order_x,2,0);
d1y_pri=d1y0;
kdy_pri=kdy0+t_min-1+(kdy0>nt1-t_min+1)*nt_inner;    %极向坐标都是从t_min开始，只包含nt_inner个点
[d1y0,kdy0]=getddiag_steady(theta(t_min:t_max),number,order_x,2,0);
d1y_SOL=d1y0;
kdy_SOL=kdy0+t_min-1;                    %极向坐标都是从t_min开始，只包含nt_inner个点
...
for i=nr_inner+1-xn_range:nr_inner
    ...
    if i>=nr_down%-- extend t before evaluation 对于私有区域的点
        ...
        d1y(i,kk(X_index),:)=d1y_pri(1,X_index,:);
...
for i=nr_inner+1:nr_inner+1+xn_range
    ...
    d1y(i,kk(X_index),:)=d1y_SOL(1,X_index,:);   kdy(i,kk(X_index),:)=kdy_SOL(1,X_index,:);
end
for j=[nt1+1:nt1+1+xn_range,nt1+nt_inner+1-xn_range:nt1+nt_inner]%本部分表示的是x点上方区域
    kk=1:nr;
    X_index=(nr_inner+1-xn_range):(nr_inner+1+xn_range);                        X_index=X_index(X_index<length(kk)&X_index>=1);
    d1x(kk(X_index),j,:) = d1x_up(X_index,1,:); kdx(kk(X_index),j,:) = kdx_up(X_index,1,:);
end
for j=  [nt1+1-xn_range:nt1,nt1+nt_inner+1:nt1+nt_inner+1+xn_range]%本部分表示的是x点下方区域
    kk=nr_down:nr;
    X_index=(nr_inner-nr_down+1+1-xn_range):(nr_inner-nr_down+1+1+xn_range);    X_index=X_index(X_index<length(kk)&X_index>=1);
    d1x(kk(X_index),j,:)=d1x_down(X_index,1,:); kdx(kk(X_index),j,:)=kdx_down(X_index,1,:);
end
%%
xn_range=5;
order_x=order+2;
...
for j=[nt1+1:nt1+1+xn_range,nt1+nt_inner+1-xn_range:nt1+nt_inner]%本部分表示的是x点上方区域
    ...
    X_index=X_index(X_index<length(kk)&X_index>=1);
    d1x(kk(X_index),j,:) = d1x_up(X_index,1,:); kdx(kk(X_index),j,:) = kdx_up(X_index,1,:);
end
for j=  [nt1+1-xn_range:nt1,nt1+nt_inner+1:nt1+nt_inner+1+xn_range]%本部分表示的是x点下方区域
    kk=nr_down:nr;
...
if ~exist("if_no_x","var")||if_no_x==0%表示求x点处有物理量含义的格式
need_change=floor(number/2);
%上方rho方向的导数系数
j=nt1+1;kk=1:nr_inner+1;
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);
...
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);%上方rho方向的导数系数
to_change=1:1+need_change;
d1x(kk(to_change),j,:)=d1x_x(to_change,:,:);
kdx(kk(to_change),j,:)=kdx_x(to_change,:,:)+kk(1)-1;
...
i=nr_inner+1;
%右上方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(nt1+1:nt1+nt_inner+1),number,order_x,2,0);
...
%左下方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(nt1+nt_inner+1:t_max),number,order_x,2,0);
kk=nt1+nt_inner+1:t_max;X_index=1:1+need_change;%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);

%右下方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(t_min:nt1+1),number,order_x,2,0);
kk=t_min:nt1+1;X_index=(nt1+1-t_min+1)-need_change:(nt1+1-t_min+1);%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);

else%求对于x点没有物理量含义的格式

need_change=floor(number/2);
%上方rho方向的导数系数
j=nt1+1;kk=1:nr_inner;
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);
to_change=nr_inner-need_change:nr_inner;
d1x(kk(to_change),j,:)=d1x_x(to_change,:,:);
kdx(kk(to_change),j,:)=kdx_x(to_change,:,:)+kk(1)-1;
%右侧rho方向的导数系数
j=nt1+1;kk=nr_inner+2:nr;
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);
to_change=1:1+need_change;
d1x(kk(to_change),j,:)=d1x_x(to_change,:,:);
kdx(kk(to_change),j,:)=kdx_x(to_change,:,:)+kk(1)-1;
%下方rho方向的导数系数
j=nt1+nt_inner+1;kk=nr_down:nr_inner;
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);
to_change=nr_inner-nr_down+1-need_change:nr_inner-nr_down+1;
d1x(kk(to_change),j,:)=d1x_x(to_change,:,:);
kdx(kk(to_change),j,:)=kdx_x(to_change,:,:)+kk(1)-1;
%左方rho方向的导数系数
j=nt1+nt_inner+1;kk=nr_inner+2:nr;
[d1x_x,kdx_x]=getddiag_steady(rho(kk),number,order_x,1,0);%上方rho方向的导数系数
to_change=1:1+need_change;
d1x(kk(to_change),j,:)=d1x_x(to_change,:,:);
kdx(kk(to_change),j,:)=kdx_x(to_change,:,:)+kk(1)-1;
...
i=nr_inner+1;
%右上方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(nt1+1+1:nt1+nt_inner+1),number,order_x,2,0);
kk=nt1+1+1:nt1+nt_inner+1;X_index=1:1+need_change;%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);

%左上方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(nt1+1:nt1+nt_inner+1-1),number,order_x,2,0);
kk=nt1+1:nt1+nt_inner+1-1;X_index=nt_inner+1-1-need_change:nt_inner+1-1;%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);

%左下方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(nt1+nt_inner+1+1:t_max),number,order_x,2,0);
kk=nt1+nt_inner+1+1:t_max;X_index=1:1+need_change;%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);

%右下方分界面
[d1y_x,kdy_x]=getddiag_steady(theta(t_min:nt1+1-1),number,order_x,2,0);
kk=t_min:nt1+1-1;X_index=(nt1+1-1-t_min+1)-need_change:(nt1+1-1-t_min+1);%x点是否更新都可以，因为不需要x点处的差分格式来计算
kdy(i,kk(X_index),:) = kdy_x(1,X_index,:)+kk(1)-1;
d1y(i,kk(X_index),:) = d1y_x(1,X_index,:);
...
function [d1,kd]=getddiag_steady(x,nd,m,x_or_y,ifperiodic)
%*****************************************************
...
if x_or_y==1%求r方向的差分时
    d1=permute(d1,[1,3,2]);kd=permute(kd,[1,3,2]);
else%求t方向的差分时
    d1=permute(d1,[3,1,2]);kd=permute(kd,[3,1,2]);
end
end
