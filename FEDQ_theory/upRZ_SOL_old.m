...
nr=eq.geometry.nr;%Due to changes in the magnetic axis and x-point magnetic flux, it is necessary to re select the radial grid distribution
...
for j=nr_down:nr_inner+1
    ...
    theta_pri_f(tr_max-1:-1:tr_min)=tf_r*(theta_div_right_new(j)-theta_pri(tr_min))+theta_pri_f(tr_max-1:-1:tr_min);%The order of these points is from near the y-axis to both sides
...
%%Make some preparations for point x here
...
%%
[x,y,npsi] = change_for_contour(x,y,npsi,eq.geometry);%The stored matrix segmentation is converted into a format that can calculate contour lines normally
...
for j=1:nr
    ...
    elseif j<=nr_inner
        ...
        else%When only internal contour lines are found, it is only possible that those corresponding to the lower boundary of the private area have not been found
            jx1=h(1,2:h(2,1)+1);%Due to being closed, the beginning and end of contour vectors are the same
            jy1=h(2,2:h(2,1)+1);
            [jx2,jy2]=rotate(eq.boundary.Rpri,eq.boundary.Zpri,R0,Z0,alpha,Length);
            [jx2,index2]=sort(jx2,'descend');jy2=jy2(index2);%Private areas can be reordered
        ...
        jtheta1=coordinate_construct(jx1,jy1);
        [jtheta1,index_j]=sort(jtheta1);jx1=jx1(index_j);jy1=jy1(index_j);%重新排序是为了插值时更合适
    ...
    elseif j==nr_inner+1%用于求解含有x点的等高线
        theta_pri_f=theta_pri_f_all(j-nr_down+1,:);
        SV=SV*[sin(alpha) cos(alpha);-cos(alpha) sin(alpha)];
        if SV(1,1)*SV(1,2)>0
            k1=-SV(2,1)/SV(2,2);%表示右上和左下的线
            k2=-SV(1,1)/SV(1,2);
        else
            k2=-SV(2,1)/SV(2,2);
            k1=-SV(1,1)/SV(1,2);
        end
        index_begin=h(1,:)==rho(j)^2;
        h(:,index_begin)=[];
        jx=h(1,:);jy=h(2,:);
        [jx,jy]= remove_repeat(jx,jy);
        para_pre=0.05:0.05:0.95;
        jx_up=jx(jy>-1&~(abs(jx)<X_x_range/sqrt(2)&jy<0));%(abs(jx)<X_x_range/2&jy<0)表示的是x点附近有待删去的点
        jy_up=jy(jy>-1&~(abs(jx)<X_x_range/sqrt(2)&jy<0));%这是为了去除掉x点附近登高面寻找错误的点，同时选择该区域对坐标点加密
        [theta_j,~]=cart2pol(jx_up,jy_up);theta_j=mod(theta_j+pi/2,2*pi);
        [~,index_j]=sort(theta_j);jx_up=jx_up(index_j);jy_up=jy_up(index_j);%重新按θ坐标排序
